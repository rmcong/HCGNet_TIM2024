...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # input = np.array(Image.fromarray(target).resize((16, 16), Image.BICUBIC)).astype('float32')
...
class DatasetFromFolderEval(data.Dataset):
    def __init__(self, lr_dir, rgb_dir, upscale_factor, input_transform=None, input_rgb_transform=None, target_transform=None):
    ...
    def __getitem__(self, index):
        ...
        target = load_img(os.path.join(self.lr_dir, os.path.splitext(file)[0] + '.png'))
        target = np.array(target).astype('float32')
        target = (target - np.min(target)) / (np.max(target) - np.min(target))

        h, w = target.shape[:2]

        input = np.array(Image.fromarray(target).resize((w // self.upscale_factor, h // self.upscale_factor), Image.BICUBIC)).astype('float32')
